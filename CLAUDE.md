...
- `call.ts` exports `grpcCall(client, method, request, metadata?)`, which promisifies grpc-js's callback API and maps gRPC status codes to `HttpError` subclasses (`NOT_FOUND`→404, `INVALID_ARGUMENT`→400, `FAILED_PRECONDITION`→409 `PRECONDITION_FAILED` (a well-formed request the resource's state forbids, message passed through verbatim — kept apart from 400 so the web can tell "fix the form" from "cannot be edited this way"), `ALREADY_EXISTS`→409 `CONFLICT`, `UNAUTHENTICATED`→401, `PERMISSION_DENIED`→403, `UNAVAILABLE`→503, else 502). Every service function funnels through this. `grpcCall` also attaches the caller's verified bearer token as `authorization: Bearer <token>` gRPC metadata, read from an `AsyncLocalStorage` request context (`src/lib/request-context.ts`, opened per request by `src/plugins/request-context.ts` and filled by `app.authenticate` after verification) — so no service signature carries it. Routes that don't run `app.authenticate` (`/health`, `/auth/login`, `/auth/register`) call upstream anonymously; an unverified `Authorization` header is never relayed.
